...
- **Deliverables:**
  ...
  2. A PR comment with an optional manual verification checklist and `docker run` instructions for the Maintainer. **If the feature requires authentication, the checklist must include login instructions** (test credentials or signup steps so the Maintainer can access the feature without prior setup).
