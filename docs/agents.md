...
7. **Developer** implements features/fixes and tests.
8. **Technical Writer** updates all relevant documentation (markdown files in the repository).
9. **Code Reviewer** reviews and approves the work. Hands off to UAT Tester for user-facing features, or directly to Release Manager for internal changes.
...
- **Key Behavior:** Follows the latest modern design standards, audits existing app patterns for consistency, and produces developer-ready design specs using the project's Tailwind/shadcn/ui design system. Never writes implementation code.
...
- **Definition of Done:** Code and tests meet requirements and pass all checks. Docker image builds and runs correctly (`docker build -t app:local -f src/Dockerfile .`).
...
| **Feature Specification** | Documents user needs, goals, and scope from an end-user perspective. Serves as the foundation for architecture and planning. Includes a mandatory "Explicit Constraints" section listing what must NOT be done — this section is propagated verbatim by all downstream agents. | Markdown document with sections: Overview, User Goals, Scope, Out of Scope, **Explicit Constraints** (mandatory — things that MUST NOT happen), User Experience, Success Criteria. | `docs/features/NNN-<feature-slug>/specification.md` |
...
| **Test Plan & Test Cases** | Defines how the feature will be verified. Maps test cases to acceptance criteria. For user-facing features, includes user acceptance scenarios for manual review. Includes a mandatory "UX Acceptance Criteria" section with product-level assertions about what the user sees, separate from functional tests. | Markdown document with: Test Objectives, Test Cases (ID, Description, Steps, Expected Result), Coverage Matrix, **UX Acceptance Criteria** (mandatory for user-facing features — product-level assertions validated by Code Reviewer and UAT Tester), User Acceptance Scenarios (for user-facing features). | `docs/features/NNN-<feature-slug>/test-plan.md` |
